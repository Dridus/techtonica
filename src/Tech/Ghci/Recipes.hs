...
import Tech.Ghci.State (currentItems, currentRecipes, fItems, fRecipes, updateState)
...
loadRecipes fp =
  loadRecipesFile fp >>= \case
    ...
    Right (warns, (its, rs)) -> do
      ...
      updateState (kw "loadRecipes" <+> viaShow fp) $ set fItems its . set fRecipes rs
...
delRecipe m rid =
  updateState (kw "delRecipe" <+> ppMachine m <+> viaShow rid) $
    over fRecipes (Map.adjust (Map.delete rid) m)
